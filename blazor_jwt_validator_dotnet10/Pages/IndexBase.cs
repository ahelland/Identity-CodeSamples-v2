...
namespace blazor_jwt_validator_dotnet10.Pages
{
    public class IndexBase : ComponentBase
    {
        ...
        private static readonly HashSet<string> TimeClaims = new(StringComparer.OrdinalIgnoreCase)
        {
            "iat", "exp", "nbf", "auth_time"
        };

        protected static string? FormatReadableUtc(string key, object? value)
        {
            if (!TimeClaims.Contains(key))
            {
                return null;
            }
            ...
            if (TryToEpoch(value, out long epochs))
            {
                try
                {
                    return DateTimeOffset.FromUnixTimeSeconds(epochs).UtcDateTime.ToString("yyyy-MM-dd HH:mm:ss \"UTC\"");
                }
                catch
                {
                    return null;
        ...
        private static bool TryToEpoch(object? value, out long epochs)
        {
            epochs = 0;
            if (value is long l) { epochs = l; return true; }
            if (value is int i) { epochs = i; return true; }
            if (value is double d) { epochs = (long)d; return true; }
            if (value is float f) { epochs = (long)f; return true; }
            if (value is string s && long.TryParse(s, out long parsed)) { epochs = parsed; return true; }
            return false;
        }
    }
}
